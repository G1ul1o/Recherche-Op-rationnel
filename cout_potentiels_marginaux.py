...
def calcul_matrice_potentiels_marginaux (graph,matrice,nbr_P,nbr_C):
    ...
    #On pose E(P(1)) =0
    index_P1 = recherche_indice(graph,"P1")
    graph[index_P1].cout_potentiel = 0 #E(P(1))

    for indices_potentiel in indices_potentiels[0]:
        index = recherche_indice(graph,("C"+str(indices_potentiel)))
        graph[index].cout_potentiel = - (graph[index_P1].cout_unitaire[indices_potentiel-1])
    ...
    while rempli==False:
        ...
        if rempli==False:
            ...
            for i in range(1,len(indices_potentiels)):
